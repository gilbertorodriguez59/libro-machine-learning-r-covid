...
|---|---|---|---|
| Video explicativo | Explicación audiovisual del proceso de descarga, revisión, limpieza y preparación de datos. | [Ver en YouTube](https://www.youtube.com/watch?v=bmv-tDPZGjQ){target="_blank"} | — |
...
| Infografía | Resumen visual de las etapas de preparación de datos. | [Ver infografía](recursos/capitulo-02/capitulo-02-preparacion-datos-reales-infografia.png){target="_blank"} | [Descargar PNG](recursos/capitulo-02/capitulo-02-preparacion-datos-reales-infografia.png){download="capitulo-02-preparacion-datos-reales-infografia.png"} |
| Cuaderno Google Colab | Cuaderno autónomo para ejecutar los ejemplos del capítulo sin necesidad de ejecutar los capítulos anteriores. | [Abrir en Google Colab](https://colab.research.google.com/github/gilbertorodriguez59/libro-machine-learning-r-covid/blob/main/colab/02-preparacion-datos.ipynb){target="_blank"} | — |
...
[**Abrir este capítulo en Google Colab**](https://colab.research.google.com/github/gilbertorodriguez59/libro-machine-learning-r-covid/blob/main/colab/02-preparacion-datos.ipynb){target="_blank"}
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      sliderInput("faltantes", "Porcentaje de valores faltantes",
                  0, 40, 15, step = 5),
...
server <- function(input, output, session) {
  ...
  output$resumen <- renderTable({
    data.frame(
      ...
      Media_variable_1 = round(c(mean(datos()[[1]], na.rm = TRUE),
                                 mean(preparados()[[1]])), 3),
      Desviacion_variable_1 = round(c(sd(datos()[[1]], na.rm = TRUE),
                                      sd(preparados()[[1]])), 3)
    )
  })
}
...
```{r}
#| label: covid-lectura-preparacion
#| message: false
#| warning: false
