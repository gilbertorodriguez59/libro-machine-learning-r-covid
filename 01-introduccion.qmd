...
| Cuaderno Google Colab | Cuaderno autónomo para ejecutar los ejemplos del capítulo sin necesidad de ejecutar los capítulos anteriores. | [Abrir en Google Colab](https://colab.research.google.com/github/gilbertorodriguez59/libro-machine-learning-r-covid/blob/main/colab/01-introduccion.ipynb){target="_blank"} | — |
...
[**Abrir este capítulo en Google Colab**](https://colab.research.google.com/github/gilbertorodriguez59/libro-machine-learning-r-covid/blob/main/colab/01-introduccion.ipynb){target="_blank"}
...
ui <- fluidPage(
  titlePanel("Clasificador basado en un umbral"),
  sidebarLayout(
    sidebarPanel(
      sliderInput("umbral", "Umbral de clasificación", 0, 10, 5, step = 0.1),
      sliderInput("ruido", "Nivel de ruido", 0, 3, 1, step = 0.1),
      numericInput("semilla", "Semilla", 123, min = 1)
    ),
    mainPanel(
      plotOutput("grafica", height = "400px"),
...
server <- function(input, output, session) {
  datos <- reactive({
    ...
    clase_real <- ifelse(x + rnorm(length(x), 0, input$ruido) >= 5, 1, 0)
    pred <- ifelse(x >= input$umbral, 1, 0)
    data.frame(x, clase_real, pred)
  })

  output$grafica <- renderPlot({
    d <- datos()
    plot(d$x, jitter(d$clase_real, amount = 0.08), pch = 19,
         xlab = "Variable predictora x", ylab = "Clase real",
         yaxt = "n", main = "Regla de clasificación")
    axis(2, at = c(0, 1), labels = c("Clase 0", "Clase 1"))
    abline(v = input$umbral, lwd = 3, lty = 2)
    grid()
  })
  ...
  output$metricas <- renderTable({
    d <- datos()
    data.frame(
      Observaciones = nrow(d),
      Aciertos = sum(d$clase_real == d$pred),
      Errores = sum(d$clase_real != d$pred),
      Exactitud = round(mean(d$clase_real == d$pred), 4)
    )
  })

  output$regla <- renderText({
    paste0("Si x >= ", round(input$umbral, 2),
           ", predecir Clase 1; en otro caso, Clase 0.")
  })
}
