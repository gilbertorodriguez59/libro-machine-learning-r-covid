...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      sliderInput("b0", "Intercepto", -10, 10, 2, 0.5),
      sliderInput("b1", "Pendiente", -5, 5, 1.5, 0.1),
      sliderInput("ruido", "Desviación del ruido", 0, 8, 2, 0.25),
      sliderInput("n", "Número de observaciones", 20, 200, 60, 10),
...
server <- function(input, output, session) {
  ...
  output$interpretacion <- renderText({
    if (input$ruido < 1) {
      "Con poco ruido, la recta estimada suele aproximarse mucho a la relación verdadera."
    } else if (input$ruido < 4) {
      "El ruido genera dispersión y reduce la precisión del ajuste."
    } else {
      "Con ruido alto, la relación es más difícil de identificar y el R² suele disminuir."
    }
  })
}
...
| Cuaderno Google Colab | Cuaderno autónomo para ejecutar los ejemplos del capítulo sin necesidad de ejecutar los capítulos anteriores. | [Abrir en Google Colab](https://colab.research.google.com/github/gilbertorodriguez59/libro-machine-learning-r-covid/blob/main/colab/04-regresion-lineal.ipynb){target="_blank"} | — |

::: {.content-visible when-format="html"}
### Video explicativo

<div class="video-responsive">
  <iframe src="https://www.youtube.com/embed/SE6DCeU9h1w" title="Capítulo 4. Regresión lineal simple y múltiple" allowfullscreen
...
[**Abrir este capítulo en Google Colab**](https://colab.research.google.com/github/gilbertorodriguez59/libro-machine-learning-r-covid/blob/main/colab/04-regresion-lineal.ipynb){target="_blank"}
